...
func main() {
	...
	// Set filename
	filename := "cafepress.com-users-11M-20Feb2019.txt"
...
func importLine(threader <-chan string, mgoreal *mgo.Session, doner chan<- bool, bar *pb.ProgressBar) {
	...
	for text := range threader {
		...
		// Check data length
		if len(data) < 2 {
			fmt.Println("length of data should be greater than two", data)
			if bar != nil {
				bar.Increment()
			}
			continue
		...
		// @bvt.bvt emails appear to be come sort of test accounts
		if strings.Contains(data[1], "@bvt.bvt") or !strings.Contains(data[1], "@") {
			if bar != nil {
				bar.Increment()
			}
			continue
