 ...
 * Hashing Method: hashcat mode 110 -> sha1($pass.$salt)
 * Hash format to crack: <password_hash>:<salt>
 */
...
func importLine(threader <-chan string, mgoreal *mgo.Session, doner chan<- bool, bar *pb.ProgressBar) {
	...
	for text := range threader {
		...
		// Check data length
		if len(data) < 4 {
			fmt.Println("length of data should be greater than 4", data)
			if bar != nil {
				bar.Increment()
			}
			continue
		...
		if !strings.Contains(data[0], "@") {
			fmt.Println("Invalid email", data[0])
			if bar != nil {
				bar.Increment()
			}
			continue
		...
		if len(data[1]) != 40 {
			fmt.Println("Invalid SHA1 hash", data[1])
			if bar != nil {
				bar.Increment()
			}
			continue
		}
		pwdHash := data[1] + ":" + data[3]
		// create our struct
		entry := MyHeritageData{
			Email:        data[0],
